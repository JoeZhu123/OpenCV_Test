...
{
  ///Classifier Parameters//下面这些参数通过程序开始运行时读入parameters.yml文件进行初始化
...
void FerNNClassifier::prepare(const vector<Size>& scales)
{
  ...
  {
	  //每一个分类器维护一个后验概率的分布，这个分布有2^d个条目（entries），这里d是像素比较pixel comparisons  
...
void FerNNClassifier::NNConf(const Mat& example, vector<int>& isin,float& rsconf,float& csconf){
   ...
   * -NN Patch
   * Outputs:
   * -Relative Similarity (rsconf)相关相似度, Conservative Similarity (csconf)保守相似度, In pos. set|Id pos set|In neg. set (isin)
   */
