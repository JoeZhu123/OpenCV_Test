...
void TLD::generatePositiveData(const Mat& frame, int num_warps)
{
  ...
  //Get Fern features on warped patches：重复利用巴恩斯利蕨类公式所产生的随机数可以形成一个蕨类形状的独特物体的特征在变形的图片上
  Mat img;
  Mat warped;
 ...
 */
  ...
  {
      idx = bad_boxes[j];//把方差较大的bad_boxes加入负样本
...
void TLD::processFrame(const cv::Mat& img1,const cv::Mat& img2,vector<Point2f>& points1,vector<Point2f>& points2,BoundingBox& bbnext,bool& lastboxfound, bool tl, FILE* bb_file)
{
  ...
  {                                       //   If NOT tracking
	  ...
	  //将聚类的cbb[0]作为新的跟踪目标box，重新初始化跟踪器  
      if(detected)
...
void TLD::learn(const Mat& img)
{
  ...
  vector<int> isin;
  float dummy, conf;//计算输入图像片（跟踪器的目标box）与在线模型之间的相关相似度conf
  classifier.NNConf(pattern,isin,conf,dummy);
  if (conf<0.5) //如果相似度太小了，就不训练：调整0.5（初始参数）这个参数看效果--0.7(两次实验发现效果不明显)
  {
...
void TLD::getOverlappingBoxes(const cv::Rect& box1,int num_closest)
{
  ...
  //Get the best num_closest (10) boxes and puts them in good_boxes：得到最好的10个邻近有界图框（bounding box）并把它们放进好的图框（good boxes）中
...
{
  TLD t;
    if (t.bbOverlap(b1,b2)<0.5)//这个参数可以改：
      return false;
    else
      return true;
}
