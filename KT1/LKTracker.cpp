...
bool LKTracker::trackf2f(const Mat& img1, const Mat& img2,vector<Point2f> &points1, vector<cv::Point2f> &points2)
{
  ...
  {
        FB_error[i] = norm(pointsFB[i]-points1[i]);//norm()求向量的最大奇异值:非负特征值的算术平方根（即A*·A的开根号值）叫作矩阵A的奇异值
  }
  //Filter out points with FB_error[i] > median(FB_error) && points with sim_error[i] > median(sim_error)
  normCrossCorrelation(img1,img2,points1,points2);
  return filterPts(points1,points2);//进行点过滤，过滤出 FB_error[i] <= median(FB_error) 和 sim_error[i] > median(sim_error) 的特征点  
}
...
{
  //Get Error Medians
  simmed = median(similarity);//取相似度的中值
  size_t i, k;
